...
r2easyR.pknot_drawer = function(R2R.sto,
  ...
  for (i in 1:length(pknot)){
    if (firstN){
      label.line[pknot[i]] <- label.index[label.index.pos]
      firstN <- FALSE
      unique.labels <- c(unique.labels, label.index[label.index.pos])
    }else{
      if (pknot[i]-1 == pknot[i -1]){
        label.line[pknot[i]] <- label.index[label.index.pos]
      }else{
  ...
  if (length(gaps) == 1){
    pknot.label.index <- c(pknot.label.index, ceiling(mean(pknot[1:(gaps-1)])), ceiling(mean(pknot[(gaps):length(pknot)])))
  }else{
